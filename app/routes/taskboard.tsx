...
import { BaseChatTaskBoard } from '~/components/chat/BaseChatTaskBoard';
import { ChatTaskBoard } from '~/components/chat/ChatTaskBoard.client';
...
export const meta: MetaFunction = () => {
  return [{ title: 'Agent8 - Task Board' }, { name: 'description', content: 'AI Game Maker - Task Board' }];
};
...
function DirectChatAccess() {
  ...
  return (
    <div className="flex flex-col h-full w-full bg-bolt-elements-background-depth-1">
      <BackgroundRays />
      <Header />
      <ClientOnly fallback={<BaseChatTaskBoard />}>{() => <ChatTaskBoard />}</ClientOnly>
    </div>
  );
}
