...
const logger = createScopedLogger('gitlab-issues-api');
...
async function issuesLoader({ context, request }: ActionFunctionArgs) {
  ...
  const state = (url.searchParams.get('state') as 'opened' | 'closed' | 'all') || 'opened';
  ...
  try {
    const result = await gitlabService.getProjectIssues(projectPath, parsedPage, parsedPerPage, state);
