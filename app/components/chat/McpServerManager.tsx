  ...
  return (
    ...
    >
      <div className="flex items-center gap-2.5 flex-wrap relative">
        {hasActiveTools && (
          <span className="text-[var(--color-text-subtle,#767D8C)] font-primary text-[14px] font-semibold leading-[142.9%] font-feature-[ss10]">
            Tools Active
          </span>
        )}
        <Tooltip.Root>
          <Tooltip.Trigger asChild>
            <button
              ...
              className={classNames(
                hasActiveTools
                  ? 'flex w-[40px] min-h-[40px] max-h-[40px] justify-center items-center rounded-[var(--border-radius-circle,99999px)] border border-solid border-[var(--color-border-interactive-neutral,rgba(255,255,255,0.18))] bg-[var(--color-bg-interactive-neutral,#222428)]'
                  : 'flex min-h-10 max-h-10 px-4 py-2.5 justify-center items-center gap-1.5 rounded-full border border-white/18 bg-[#222428] text-sm font-medium hover:text-gray-500',
                'transition-colors duration-200',
              )}
            >
              <img src="/icons/Plus.svg" alt="Plus" className={hasActiveTools ? 'w-5 h-5' : ''} />
              {!hasActiveTools && <span className="font-normal text-cyan-400">Use Tools</span>}
            </button>
          </Tooltip.Trigger>
          <Tooltip.Portal>
            <Tooltip.Content
              className="inline-flex items-start rounded-radius-8 bg-[var(--color-bg-inverse,#F3F5F8)] text-[var(--color-text-inverse,#111315)] p-3 shadow-md z-[9999] font-primary text-base font-medium leading-[150%] w-[292px] justify-between"
              sideOffset={5}
        ...
        {mcpServers
          ...
          .map(({ server, index }) => (
            <div
              key={index}
              className="flex min-h-10 max-h-10 px-[var(--spacing-16,16px)] py-[var(--spacing-10,10px)] justify-center items-center gap-[var(--spacing-6,6px)] rounded-[var(--border-radius-circle,99999px)] border border-solid border-[var(--color-border-interactive-neutral-hovered,rgba(255,255,255,0.22))] text-sm font-medium text-gray-800 dark:text-gray-200 cursor-pointer"
              title={server.url}
        ...
        {showServerManager && (
          <motion.div
            ref={dropdownRef}
            className={classNames(
              'absolute left-0 flex w-[356px] py-[var(--spacing-8,8px)] px-0 flex-col items-start rounded-[var(--border-radius-8,8px)] border border-solid border-[var(--color-border-tertiary,rgba(255,255,255,0.12))] bg-[var(--color-bg-interactive-neutral,#222428)] z-10',
          ...
          >
            {mcpServers.length > 0 ? (
                    ...
                    >
                      <div className="flex items-center gap-6">
                        <button
                          type="button"
                          className={classNames(
                            'flex w-5 h-5 p-[var(--spacing-0,0px)] flex-col items-start gap-[var(--spacing-0,0px)] aspect-square rounded-[var(--border-radius-2,2px)] cursor-pointer',
                            server.enabled
                        ...
                        <div className="flex flex-col justify-center items-start gap-2 flex-1">
                          <div className="flex items-center gap-2 self-stretch">
                            ...
                            <div className="flex items-center gap-1.5">
                              <h4 className="text-[var(--color-text-primary,#FFF)] font-primary text-base font-medium leading-[150%]">
                                {server.name}
                              </h4>
                          ...
                          <div>
                            <p className="text-[14px] font-primary font-medium leading-[142.9%] text-[var(--color-text-tertiary,#99A2B0)]">
                              {server.description}
                            </p>
                          </div>
                        </div>
              ...
              <div className="w-full text-center py-4 px-4 text-bolt-elements-textSecondary text-sm">
