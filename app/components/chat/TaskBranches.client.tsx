...
const TaskBranch = ({ branch, onRemove }: { branch: any; onRemove: () => Promise<void> }) => {
  ...
  return (
    <div className="flex w-full flex-1 max-w-chat mx-auto max-h-22 mb-3 rounded-lg border border-purple-400 bg-gradient-to-r from-purple-900/90 to-indigo-900/90 p-4 shadow-md backdrop-blur-sm transition-all hover:border-purple-300 hover:shadow-lg">
      ...
      <div className="flex items-center ml-3 flex-shrink-0 gap-1.5">
        <button
          className="px-4 py-1.5 bg-transparent text-purple-100 rounded-md hover:bg-purple-100 hover:bg-opacity-20 transition-colors shadow-sm font-medium text-sm disabled:opacity-50 disabled:cursor-not-allowed"
          onClick={handleClose}
          disabled={isLoading}
        >
          {isLoading ? (
