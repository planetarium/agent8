...
import { BaseChatTaskBoard, type ChatAttachment } from './BaseChatTaskBoard';
...
} from '~/lib/persistenceGitbase/api.taskboard.client';
import { container } from '~/lib/container';
import { DEFAULT_TASK_BRANCH, repoStore } from '~/lib/stores/repo';
import type { FileMap } from '~/lib/.server/llm/constants';
import { useGitbaseChatHistory } from '~/lib/persistenceGitbase/useGitbaseChatHistoryTaskBoard';
...
export function ChatTaskBoard() {
  renderLogger.trace('ChatTaskBoard');
  ...
  useEffect(() => {
    if (repoStore.get().path) {
      ...
      changeChatTaskBoardUrl(repoStore.get().path, { replace: true, searchParams: {}, ignoreChangeEvent: true });
    }

    // TaskBoard 페이지에서는 repoStore.path가 없을 때 URL을 변경하지 않습니다
  }, []);
      ...
      if (!chatStarted) {
        try {
          ...
          if (isEnabledGitbasePersistence) {
            ...
            changeChatTaskBoardUrl(projectPath, { replace: true });
          } else {
            repoStore.set({
              name: projectRepo,
              path: projectRepo,
            ...
            changeChatTaskBoardUrl(projectRepo, { replace: true });
          }
    ...
    const handleFork = async (message: Message) => {
      ...
      try {
        ...
        if (forkedProject && forkedProject.success) {
          ...
          window.location.href = '/chat-taskboard/' + forkedProject.project.path;
        } else {
